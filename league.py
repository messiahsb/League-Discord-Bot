...
def get_puuid(gameName:str = None, tagLine : str = None) -> str: 
    ...
    id_request = requests.get(ROOT + id_endpoint + "?api_key=" + L_API) 
...
def get_player(puuid:str = None) -> dict[str, str]: 
    ...
    player_request = requests.get(ROOT + player_endpoint + "?api_key=" + L_API) 
...
def get_match_history(puuid:str = None, start: str = 0, count: str = 20) -> list:
    ...
    match_request = requests.get(ROOT + match_endpoint + "&api_key=" + L_API) 
